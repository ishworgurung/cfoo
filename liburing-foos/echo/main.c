...
void add_provide_buf(struct io_uring *ring, unsigned short bid, unsigned gid);
...
int main(int argc, char *argv[]) {
    if (argc < 2) {
        printf("Please give a port number: ./echo [port]\n");
        exit(0);
    }
    ...
    if ((params.features & IORING_FEAT_FAST_POLL) == 0x1) {
